...
class ApplicationModule(QObject):
    ...
    def do_init_checks_menu(self):
        ...
        menu = QMenu(menubar)
        menu.setTitle(_translate("VeriSO_V+D", "Checks",  None))
        ...
        if not check_topics:
            ...
            self.iface.messageBar().pushMessage("Error",   _translate("VeriSO_V+D", message, None), level=QgsMessageBar.CRITICAL, duration=10)   
            return
    ...
    def do_show_complex_check(self, check):
        try:
            ...
            QMessageBox.critical(None, "VeriSO_V+D", str(traceback.format_exc(exc_traceback)))               
            return
    ...
    def do_init_baselayer_menu(self):
        ...
        menubar.setSizePolicy(QSizePolicy(QSizePolicy.Minimum, QSizePolicy.Minimum))
        menu = QMenu(menubar)
        menu.setTitle(_translate("VeriSO_V+D", "Baselayer",  None))  
    ...
    def do_init_topics_tables_menu(self):
        ...
        menubar.setSizePolicy(QSizePolicy(QSizePolicy.Minimum, QSizePolicy.Minimum))
        menu = QMenu(menubar)
        menu.setTitle(_translate("VeriSO_V+D", "Tables", None))  
        ...
        if not topics:
            ...
            QMessageBox.critical(None, "VeriSO_V+D", self.tr(message))
            return

        for topic in topics:
            topic_menu = menu.addMenu(unicode(topic["topic"]))      
            
            action = QAction(_translate("VeriSO_V+D", "Load Topic",  None), self.iface.mainWindow())
    ...
    def get_topics_tables(self):
        ...
        try:            
            ...
            if not db.open():
                ...
                QgsMessageLog.logMessage(self.tr(message) + db.lastError().driverText(), "VeriSO_V+D", QgsMessageLog.CRITICAL)                                
                return
            ...
            if not query.isActive():
                ...
                QgsMessageLog.logMessage(self.tr(message), "VeriSO_V+D", QgsMessageLog.CRITICAL)            
                QgsMessageLog.logMessage(str(QSqlQuery.lastError(query).text()), "VeriSO_V+D", QgsMessageLog.CRITICAL)      
                return 
    ...
    def do_init_defects_menu(self):
        ...
        menubar.setSizePolicy(QSizePolicy(QSizePolicy.Minimum, QSizePolicy.Minimum))
        menu = QMenu(menubar)
        menu.setTitle(_translate("VeriSO_V+D", "Defects",  None))  

        action = QAction(_translate("VeriSO_V+D", "Load defects layer",  None), self.iface.mainWindow())
        QObject.connect(action, SIGNAL( "triggered()"), lambda foo="bar": self.do_load_defects(foo))
        menu.addAction(action)     
        
        action = QAction(QCoreApplication.translate("VeriSO_V+D", "Export defects layer"), self.iface.mainWindow())
