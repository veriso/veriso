...
GRANT ALL ON TABLE $$DBSCHEMA.einzelobjekte_punktelement_v TO $$USER;'),
 (105,'CREATE OR REPLACE VIEW $$DBSCHEMA.fixpunktekategorie3_lfp3_ausserhalb_perimeter_v AS 
...
'),
 (106,'CREATE OR REPLACE VIEW $$DBSCHEMA.fixpunktekategorie3_lfp3_pro_toleranzstufe_v AS 
...
GRANT ALL ON TABLE $$DBSCHEMA.bodenbedeckung_objektnamepos_v TO $$USER;'),
 ...
 SELECT sym.ogc_fid,
  ...
  WHERE sym.boflaechesymbol_von::text = bb.tid::text;
...
WHERE boden.art = ''Gebaeude'') touches
ON touches.ogc_fid = elem.ogc_fid WHERE touches.ogc_fid IS NULL AND obj.art = ''uebriger_Gebaeudeteil'';
...
WHERE 
  ...
  liegenschaften_grundstueckpos.grundstueckpos_von = liegenschaften_grundstueck.tid) as foo
...
SELECT distinct a.ogc_fid, a.tid, ST_GeometryFromText((((((((''LINESTRING(''::text || ST_X(a.lage)::text) || '' ''::text) || ST_Y(a.lage)::text) || '',''::text) || ST_X(c.pos)::text) || '' ''::text) || ST_Y(c.pos)::text) || '')''::text, $$EPSG) AS line, a.hausnummer
